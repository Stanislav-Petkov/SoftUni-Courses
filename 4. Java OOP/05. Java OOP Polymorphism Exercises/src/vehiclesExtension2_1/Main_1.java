...
public class Main_1 {
    public static void main(String[] args) {
        ...
        Map<String, Vehicle_1> vehiclesByType = new LinkedHashMap<>();
        ...
        //bus input
        input = scanner.nextLine().split("\\s+");
        vehiclesByType.put("Bus", new Bus_1(Double.parseDouble(input[1]), Double.parseDouble(input[2]),
                Double.parseDouble(input[3])));
        ...
        while (n-- > 0) {
            ...
            if (tokens[0].equals("Drive")) {
                ...
                System.out.println(vehiclesByType.get(tokens[1]).driveBusEmpty(param));
            }
        }
        for (Vehicle_1 vehicle : vehiclesByType.values()) {
