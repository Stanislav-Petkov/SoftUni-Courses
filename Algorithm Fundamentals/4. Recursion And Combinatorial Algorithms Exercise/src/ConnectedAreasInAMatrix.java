...
public class ConnectedAreasInAMatrix {
    ...
    public static void main(String[] args) {
        ...
        for (int row = 0; row < matrix.length; row++) {
            for (int col = 0; col < matrix[row].length; col++) {
                if (matrix[row][col] == '-') {
                    // When a new area is found a list with starting point and area size number is generated
    ...
    private static void findAreas(int row, int col) {
        ...
        // Going down in the recursion mark every sell as visited
        matrix[row][col] = 'V';

        // Get the array at the last position and increment the area size counter at 2nd position with 1
