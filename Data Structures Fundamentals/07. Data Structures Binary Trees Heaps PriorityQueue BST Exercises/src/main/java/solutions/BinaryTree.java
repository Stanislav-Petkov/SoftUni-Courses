...
public class BinaryTree {
    ...
    private boolean findNodePath(BinaryTree binaryTree, int element, List<Integer> currentPath) {
        ...
        boolean rightResult = this.findNodePath(binaryTree.right, element, currentPath);
        if (rightResult) {
            return true;
        }
    ...
    public List<Integer> topView() {
        ...
        this.traverseTree(this, 0, 1, offsetToValueLevel);
    ...
    private void traverseTree(BinaryTree binaryTree, int offset, int level,
        ...
        Pair<Integer, Integer> currentValue = offsetToValueLevel.get(offset);

        if (currentValue == null || level < currentValue.getValue()) {
            offsetToValueLevel.putIfAbsent(offset, new Pair<>(binaryTree.value, level));
        }
